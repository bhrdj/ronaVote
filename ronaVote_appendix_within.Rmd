...
regressEachWeekCol <- function(df, sR){
  
  # sR <- "TX"
  # df <- ronaPer1000_byStateOrRegion %>% filter(stateOrRegion == sR)
...
lmOut_betaCoeffs <- bind_rows(lmOut_stateOrRegion, .id = "column_label") %>%
  ...
  mutate(ronaVoteTrend = ifelse(pVal_coeff < .05 & coeff > 0, "red", 
                       ifelse(pVal_coeff < .05 & coeff < 0, "blue", NA))
         ) 

signif_plot <- lmOut_betaCoeffs %>%
