...
girafe(code = print(plot_byState), width_svg = 8, height_svg = 4)
...
## Setup "within" and "between" regressions
```{r eval=TRUE}
...
stateOrRegions_only <- ronaPer1000_byStateOrRegion$stateOrRegion %>% 
  unique() 

```
## Function for "within" regressions
```{r}
...
for (sR in stateOrRegions_only) {
  lmOut_stateOrRegion[[sR]] <- ronaPer1000_byStateOrRegion %>% 
    filter(stateOrRegion == sR) %>%
    regressEachWeekCol(sR)
}

# EXPORT TO .RData
myVec <- ls()[!ls() %in% c("lmOut_stateOrRegion",
                           "weekDates",
                           "stateOrRegions_only",
                           "ronaPer1000_byStateOrRegion")]
rm(list = myVec)

```
## Plot "within" regressions
```{r warning=FALSE, fig.height=8}

load("./temp_workspace/lmOut_stateOrRegion.RData")
...
lmOut_betaCoeffs %>%
  ...
  geom_point(size = 0.7) +
  scale_colour_manual(values = c(Republican = "#BB3333", Democrat = "#3333BB")) + 
  labs(title="Individual StateOrRegion Regression Coefficients Over Time",
       ...
       caption=paste("State/Regions on weeks where (β>0) & (β=0 with p<0.05) have a red dot.\n",
                     "(β<0) & (β=0 with p<0.05) have a blue dot.\n",
                     "When (p>0.5), then no dot is plotted.")) +
  dark_theme_gray()
   
```

## Run and plot "between" regression
```{r}
rm(lmOut_betaCoeffs, lmOut_stateOrRegion, myVec, fips_stateOrRegion, plot_byState, plot_byStateOrRegion, ronaPer1000)

x <- ronaPer1000_byStateOrRegion %>% 
  group_by(stateOrRegion) %>%
  summarize(DJT_Margin = weighted.mean(DJT_Margin, (REPUBLICAN+DEMOCRAT)),
            countyCounts_sR = n(),
            across(starts_with("T", ignore.case=FALSE), ~ weighted.mean(., pop2019)),
            pop2019 = sum(pop2019),
            )

xx <- regressEachWeekCol(x, 1) %>% inner_join(weekDates)

ggplot(xx, aes(y=coeff, x=weekDate)) +geom_point()
