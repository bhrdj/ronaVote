...
rm(ronaDays)
weekDates <- distinct(select(ronaWeeks, weekDate, week_DateT))                  # Put aside weekDates in advance of spread()
...
tidyIt <- function(ronaTall, area, popu, vote) {
  ...
  ronaWideW <- pivot_wider(data = ronaTall, id_cols = fips, 
                          names_from = week_DateW, values_from = casesByPopW)    # Only 1 row/county; 1 col/week. Weekly new cases
