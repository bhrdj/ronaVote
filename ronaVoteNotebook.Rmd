...
The maximum correlation of Democratic areas and cumulative severity of COVID occurred the second week of August 2020, at 0.00008 COVID-cases per percentage point of vote margin. (8 per 100,000 population) By the second week of November, the Democratic and Republican areas had accumulated balanced counts, at the moment of the election. The maximum correlation of Republican areas with cumulative COVID cases occurred in the second or third week of January 2021, at .00017 COVID-cases per percentage point of vote margin. (17 per 100,000)
...
ronaSectionsTot <- ronaSectionsTot_BeforeCleaning %>%
  filter(!(fips %in% countiesWithNA_fips))
...
lm_allT <-  map(select(ronaPerCap, starts_with("T")), 
                       function(yvar) {
...
# colSums(ronaSectionsTot[
#   c("pop2019", "totalvotes", "REPUBLICAN", "DEMOCRAT")]) %>%
#   add_column()
# ronaTtlPerCapUSA <- ronaTotalUSA %>% 
  


# Join additional columns for analysis
coeff_R2_p_df <- lm_dfT %>% 
...
Margin_vs_CovidPerCap <- coeff_R2_p_df %>% 
...
maxRepubCorr <- max(coeff_R2_p_df$BetaCoeff)
maxDemocCorr <- min(coeff_R2_p_df$BetaCoeff)
balanceCorr <- min(abs(coeff_R2_p_df$BetaCoeff))

coeff_R2_p_df %>% filter(BetaCoeff %in% c(maxRepubCorr, maxDemocCorr, balanceCorr))
...
## To put this in context, there were 4,795,395 /  cumulative USA cases per 100,000 in the second week of August 2020, and *** per 100,000 in January 2021.
